...
# TODO Revisar
def evaluate_linear_f1(linear_model, X_test, y_test):
    linear_pred = linear_model.predict(X_test)
    linear_pred = np.where(np.array(linear_pred) <= 0.5, 0, 1)
...
# TODO Revisar
def evaluate_qda_accuracy(qda_model, X_test, y_test):
...
def load_mlp(hidden_layer_sizes=(10,), activation="relu"):
    return MLPClassifier(hidden_layer_sizes=hidden_layer_sizes, activation=activation)
...
# TODO Revisar
def evaluate_mlp_accuracy(mlp_model, X_test, y_test):
...
# TODO Revisar importante
def evaluate_kmeans_accuracy(kmeans_model, X_test, y_test):
